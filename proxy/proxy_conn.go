...
import (
	...
	"github.com/gameparrot/mcpelauncher_pcproxy/utils"
...
func (c *ProxyConn) SetSaltAndKey() {
	c.salt = make([]byte, 16)
	rand.Read(c.salt)

	c.privateKey, _ = ecdsa.GenerateKey(elliptic.P384(), rand.Reader)
}

func (c *ProxyConn) ReadLoop() error {
	for {
		pks, err := c.dec.Decode()
		if err != nil {
			c.conn.Close()
			return err
		}
		for _, pk := range pks {
			packetData, err := ParseData(pk)
			if err != nil {
				c.conn.Close()
				return err
			}
			if pkFunc, ok := c.pool[packetData.h.PacketID]; ok {
				...
				case *packet.Login:
					...
					if c.nethernet || c.privateKey == nil {
						return nil
					}
				...
				case *packet.PlayStatus:
					if pk.Status == packet.PlayStatusLoginSuccess {
						return nil
					}
				}
			}
...
func (p *ProxyConn) ReadCompressedPacket() ([]byte, error) {
	...
	if !p.nethernet {
		pk = pk[1:]
		if p.decrypt != nil {
			p.decrypt.decrypt(pk)
			err = p.decrypt.verify(pk)
			pk = pk[:len(pk)-8]
		}
...
func (p *ProxyConn) WriteCompressedPacket(b []byte) (int, error) {
	if !p.nethernet && p.encrypt != nil {
		b = p.encrypt.encrypt(b)
	}
	return p.conn.Write(b)
}

func (p *ProxyConn) Conn() net.Conn {
	return p.conn
}
...
func (conn *ProxyConn) handleLogin(pk *packet.Login) error {
	...
	if !conn.nethernet && conn.privateKey != nil {
...
func (conn *ProxyConn) enableEncryption(clientPublicKey *ecdsa.PublicKey) error {
	...
	conn.prepareEncryption()

	return nil
}

func (conn *ProxyConn) prepareEncryption() {
	enc := utils.FetchPrivateField[any](conn.enc, "encrypt")
	conn.encrypt = utils.UnsafeCast[encrypt](enc)

	dec := utils.FetchPrivateField[any](conn.dec, "encrypt")
	conn.decrypt = utils.UnsafeCast[encrypt](dec)
...
func (conn *ProxyConn) WritePacket(pk packet.Packet) error {
	...
	return conn.enc.Encode([][]byte{b.Bytes()})
}
...
func (conn *ProxyConn) handleServerToClientHandshake(pk *packet.ServerToClientHandshake) error {
	...
	b := bytes.NewBuffer(nil)
	conn.hdr.PacketID = packet.IDClientToServerHandshake
	_ = conn.hdr.Write(b)

	io := protocol.NewWriter(b, 0)
	(&packet.ClientToServerHandshake{}).Marshal(io)

	buf := &bytesBufferWithPacketHeader{header: 254, writeHeader: !conn.nethernet, Buffer: bytes.NewBuffer(nil)}
	enc := packet.NewEncoder(buf)
	enc.EnableCompression(conn.compression)
	enc.Encode([][]byte{b.Bytes()})
	conn.WriteCompressedPacket(buf.Bytes())
...
type bytesBufferWithPacketHeader struct {
	header      byte
	writeHeader bool
	*bytes.Buffer
}

func (b *bytesBufferWithPacketHeader) PacketHeader() (byte, bool) {
	return b.header, b.writeHeader
}
